...
class KDiffusionSampler:
    ...
    def sample(self, S, conditioning, batch_size, shape, verbose, unconditional_guidance_scale, unconditional_conditioning, eta, x_T, callback=NullCallback):
...
def process_images(
        ...
        variant_amount=0.0, variant_seed=None,imgProcessorTask=False, job_info: JobInfo = None, callback = NullCallback):
    ...
    with torch.no_grad(), precision_scope("cuda"), (model.ema_scope() if not opt.optimized else nullcontext()):
        ...
        for n in range(n_iter):
            ...
            def cb(i):
                try:
                    i['iter'] = n
                    i['total_iters'] = n_iter
                    i['total_steps'] = steps
                    return callback(i)
                except Exception as err:
                    print(f"Error during callback: {err}")



            samples_ddim = func_sample(init_data=init_data, x=x, conditioning=c, unconditional_conditioning=uc, sampler_name=sampler_name, callback=cb)
...
def txt2img(prompt: str, ddim_steps: int, sampler_name: str, toggles: List[int], realesrgan_model_name: str,
            ...
            height: int, width: int, fp, variant_amount: float = None, variant_seed: int = None, job_info: JobInfo = None, callback = NullCallback):
    print(prompt,
          ddim_steps,
          sampler_name,
          toggles,
    ...
    def sample(init_data, x, conditioning, unconditional_conditioning, sampler_name, callback = NullCallback):
...
def img2img(prompt: str, image_editor_mode: str, mask_mode: str, mask_blur_strength: int, ddim_steps: int, sampler_name: str,
            ...
            seed: int, height: int, width: int, resize_mode: int, init_info: any = None, init_info_mask: any = None, fp = None, job_info: JobInfo = None, callback = NullCallback):
    print([prompt, image_editor_mode, init_info, init_info_mask, mask_mode,
                                   ...
                                   fp])
    init_info_mask["image"].save("init_image.png")
    init_info_mask["mask"].save("init_mask.png")
    ...
    def sample(init_data, x, conditioning, unconditional_conditioning, sampler_name, callback=NullCallback):
        ...
        if sampler_name != 'DDIM':
            ...
            samples_ddim = sampler.decode(z_enc, conditioning, t_enc_steps,
                                            ...
                                            z_mask=z_mask, x0=x0, callback=callback)
        return samples_ddim
    ...
    def processGoBig(image):
        ...
        def sample(init_data, x, conditioning, unconditional_conditioning, sampler_name, callback=NullCallback):
            if sampler_name != 'DDIM':
                ...
                samples_ddim = K.sampling.__dict__[f'sample_{sampler.get_sampler_name()}'](model_wrap_cfg, xi, sigma_sched, extra_args={'cond': conditioning, 'uncond': unconditional_conditioning, 'cond_scale': cfg_scale}, disable=False, callback=callback)
            else:
                x0, = init_data
                sampler.make_schedule(ddim_num_steps=ddim_steps, ddim_eta=0.0, verbose=False)
